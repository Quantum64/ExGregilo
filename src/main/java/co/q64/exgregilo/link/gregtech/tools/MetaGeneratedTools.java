...
public class MetaGeneratedTools extends GT_MetaGenerated_Tool {
	...
	public void addAdditionalToolTips(List list, ItemStack stack, EntityPlayer player) {
		...
		if (stats != null) {
			...
			list.add(tOffset + 2, EnumChatFormatting.WHITE + "Clicks Required: " + EnumChatFormatting.LIGHT_PURPLE + Math.max(1, stats.getSpeedMultiplier() * (WireMesh.MAX_SPEED - getPrimaryMaterial(stack).mToolSpeed)) + EnumChatFormatting.GRAY);
		}
	}
